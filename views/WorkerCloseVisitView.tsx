...
interface WorkerCloseVisitViewProps {
  onBack: () => void;
  onConfirmClose: () => void;
}

export const WorkerCloseVisitView: React.FC<WorkerCloseVisitViewProps> = ({ onBack, onConfirmClose }) => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 font-sans h-screen overflow-hidden text-slate-900">
      ...
      <header className="flex-none bg-slate-50 px-4 py-3 flex items-center justify-between border-b border-slate-200 z-30 transition-colors">
        ...
        <h2 className="text-slate-900 text-lg font-bold leading-tight tracking-tight flex-1 text-center mx-2">Cierre de Visita</h2>
        ...
        <section className="p-4 pt-6">
          ...
          <div className="flex flex-col">
            <div className="group relative flex flex-col items-center gap-4 rounded-xl border-2 border-dashed border-[#d5d9e2] bg-white px-6 py-10 transition-all hover:border-slate-900 cursor-pointer active:bg-slate-50">
              <div className="h-14 w-14 rounded-full bg-[#eaecf1] flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
          ...
          <div className="relative">
            ...
            <div className="flex justify-between items-center mt-2 px-1">
              <div className="flex items-center gap-1.5">
                <div className={`h-2 w-2 rounded-full ${description.length < minChars ? 'bg-red-500 animate-pulse' : 'bg-emerald-500'}`}></div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-slate-200 z-40 max-w-md mx-auto">
        <div className="p-4">
          <button 
            ...
            className="w-full h-14 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-xl shadow-lg shadow-slate-900/20 flex items-center justify-center gap-2 text-base transition-all hover:brightness-110 active:scale-[0.98] group"
          >
